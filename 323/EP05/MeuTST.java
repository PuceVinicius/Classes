...
public class MeuTST<Value extends Comparable<Value>> {
    ...
    private static class StringV<Value extends Comparable<Value>> implements Comparable<Value> {
        private String str;						// left, middle, and right subtries
        private Value val;                     	// value associated with string
    
    @Override
    public int compareTo (StringV<Value> that) {
    	if(this.val.compareTo(that.val) < 0){
                return 1;
            }

            else if (this.val.compareTo(that.val) > 0){
                return -1;
            }
   ...
   public Iterable<String> keysWithPrefixByValue(String prefix) {
        ...
        if (x == null) return queue;
        if (x.val != null) queue.enqueue(prefix);
        collect(x.mid, queue, new StringBuilder(prefix));

        StringV disney = new StringV<Value>()
        MinPQ<StringV> queueV = new MinPQ<StringV>();
        while(queue.hasNext()) {
        	disney.str = queue.next();
        	disney.val = this.get(disney.str);
        	queueV.insert(disney);
        }

        Queue<String> queueF = new Queue<String>();
        for (StringV lul : queueV) {
        	queueF.enqueue(lul.str);
        }
        return queueF;
    }

    // all keys in subtrie rooted at x with given prefix
    private void collect(Node<Value> x, Queue<String> queue, StringBuilder prefix) {
        if (x == null) return;
    ...
    private Node<Value> delete(Node<Value> x, String key, int d) {
        ...
        if (this.isEmpty(x) == null) return null;
        else return x;
    }

    //retorna null se for vazio
    private Node<Value> isEmpty(Node<Value> x) {
    	if (x.right == null && x.left == null && x.mid == null) return null;
    	else return x;
    }

    /*private Node<Value> destroy(Node<Value> x) {
