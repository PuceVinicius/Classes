...
CREATE OR REPLACE FUNCTION delete_administra (aCurso INT, aPeriodo date, aNUSP INT) returns void
	LANGUAGE plpgsql
	AS $$
	BEGIN
		DELETE FROM "Administra"
		WHERE "NUSP" = aNUSP AND "Curso" = aCurso AND "Prd" = aPeriodo;
	END;
	$$;
...
CREATE OR REPLACE FUNCTION delete_oferecimento (oid_pessoa INT, oDisc INT, oPRD date) returns void
	...
	BEGIN
		...
		WHERE "ID_Pessoa" = oid_pessoa AND "Disc_ID" = oDISC AND "Prd" = oPRD;
	END;
	$$;
