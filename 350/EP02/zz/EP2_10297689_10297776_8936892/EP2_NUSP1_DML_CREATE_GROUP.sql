...
CREATE OR REPLACE FUNCTION insert_us_pf (pPerfil INT, uus_id INT) returns void
	LANGUAGE plpgsql
	AS $$
	BEGIN
...
CREATE OR REPLACE FUNCTION insert_admin (aid_pessoa INT, aPerfil INT, aNUSP INT) returns void
	...
	BEGIN
		...
		VALUES (pNUSP, aPerfil, aid_pessoa);
	END;
	$$;
...
CREATE OR REPLACE FUNCTION insert_drel_cur_tri (rCurso INT, rID_trilha INT, rDisc INT) returns void
	LANGUAGE plpgsql
	AS $$
	BEGIN
