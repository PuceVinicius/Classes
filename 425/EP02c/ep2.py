    ...
    def get_action(self, perception):
        ...
        for x in range(0, len(grid)):
            for y in range(0, len(grid[0])):
                if grid[x][y] in playnum:
                    self_x = x
                    self_y = y
        total_actions.append('STOP')
        if grid[self_x][self_y] == 8 or grid[self_x][self_y] == 9:
        ...
        if self.player_number == 1:
            ...
            cant_go = [1, 5, 8]
        if self_x != 0:
            if grid[self_x-1][self_y] not in cant_go:
                total_actions.append('UP')
        if self_x != (len(grid)-1):
            if grid[self_x+1][self_y] not in cant_go:
                total_actions.append('DOWN')
        if self_y != 0:
            if grid[self_x][self_y-1] not in cant_go:
                total_actions.append('LEFT')
        if self_y != (len(grid[0])-1):
            if grid[self_x][self_y+1] not in cant_go:
...
class CollectAllAgent(util.Agent):
    ...
    def heuristic(self, node):
        goals = self.problem.all_people(node.state)
        state = node.state[0]
        best_distance = util.INT_INFTY
        for people in goals:
            heur = abs(state[0]-people[0])+abs(state[1]-people[1]) #manhattan
            if heur < best_distance:
                best_distance = heur
    ...
    def __state_from_perception(self, perception):
        grid_l, remaining_gas = perception
        # Car and car parked on the gas station
        player_values = [self.player_number, self.player_number+7]
        grid = tuple(map(tuple, grid_l))

        for i in range(len(grid_l)):
            for j in range(len(grid_l[0])):
                if grid_l[i][j] in player_values:
                    return ((i, j), remaining_gas, grid)
        return None
...
class CollectAllAgentProblem(util.Problem):
    ...
    def next_state(self, state, action):
        ...
        i, j = ag_pos
        newgrid = [list(i) for i in state[2]]
        aux = {'UP'    : (i-1, j),
               'DOWN'  : (i+1, j),
               'LEFT'  : (i, j-1),
               'RIGHT' : (i, j+1),
        ...
        if action not in self.actions(state):
            return (aux['STOP'], remaining_gas - self.cost(state, action), state[2])
        new_i, new_j = aux[action]
        if action == 'REFILL':
            if remaining_gas + util.DEFAULT_REFILL > self.tank_capacity:
                newgrid[new_i][new_j] = player_number + 7
                grid = tuple(map(tuple, newgrid))
                return (aux['REFILL'], self.tank_capacity, grid)
            else:
                newgrid[new_i][new_j] = player_number + 7
                grid = tuple(map(tuple, newgrid))
                return (aux['REFILL'], remaining_gas + util.DEFAULT_REFILL, grid)
        if newgrid[new_i][new_j] == 4:  # Agent going to a gas station
            newgrid[new_i][new_j] = player_number + 7
        else:
            newgrid[new_i][new_j] = player_number

        grid = tuple(map(tuple, newgrid))
        return (aux[action], remaining_gas - self.cost(state, action), grid)

    def all_people(self, state):
        ...
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                if grid[i][j] in people_numbers:
                    people_pos[(i, j)] = grid[i][j]
        return people_pos

    def is_goal_state(self, state):
        i, j = state[0]
        grid = state[2]
        people = self.all_people(state)
        if not people:
            return True
        else:
            return False
    ...
    def __process_state(self, state):
        ...
        :rtype: <class 'tuple'>
        """
        agent_pos, remaining_gas, grid = state
        i, j = agent_pos
    ...
    def my_better_evaluation_function(self, state, player=1):
        ...
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                if grid[i][j] in people_numbers:
                    people_pos[(i, j)] = grid[i][j]
                if grid[i][j] in p_num:
                    player_pos[0] = i
                    player_pos[1] = j

        for people in people_pos:
            heur -= abs(player_pos[0]-people[0])+abs(player_pos[1]-people[1]) #manhattan
        _, _, _, _, a1p, a2p = state
        if player == 1:
            return a1p + heur
        else:
