"""
  ...
  Referencias: 
  A implementacao da fila de prioridade
...
def word_count(filename):
    ...
    #Funcao adicional para calcular o numero de palavras em um arquivo

    file = open(filename, "r")
    raw_str = file.read()
    file_str = raw_str.center(len(raw_str) + 2)
    ...
    for j in range(0, len(file_str)):
        if (not file_str[j].isalnum()):
            if (strBool):
                wordCount += 1
                strBool = False
            i = j
...
def compute_word_freq(filename):
    ...
    """
    O algoritmo itera sobre cada caracter da string. Enquanto o
    caracter for alfanumerico, a variavel booleana strBool ficara como
    verdadeira. No primeiro caracter nao alfanumerico depois, o algoritmo
    fara um check pra ver se ela e' uma string e se tem 5 ou mais caracte-
    res, assim colocando no dicionario freq. Depois, conta-se todas pala-
    vras do arquivo e divide cada um dos valores do dicionario pela 
    ...
    i = 0
    j = 0
    freq = {} #inicializacao do dicionario
    strBool = False

    file = open(filename, "r")
    raw_str = file.read()
    file_str = raw_str.center(len(raw_str) + 2).lower()

    for j in range(0, len(file_str)):
        if (not file_str[j].isalnum()):
            if (strBool):
                if (j - i >= 6):
                    if file_str[i+1:j] in freq:
                        temp = freq.get(file_str[i+1:j])
                        freq.pop(file_str[i+1:j])
                        freq[file_str[i+1:j]] = temp + 1
                    else:
                        freq[file_str[i+1:j]] = 1
                strBool = False
            i = j
        else:
            strBool = True
    ...
    freq = {k: v/wordCount for k, v in freq.items()}

    return freq

def compute_phrase_freq(filename):
    """
    Unica coisa diferente do algoritmo anterior e' que para identificar
    palavras seguidas, mantem-se duas variaveis booleanas para checar
    se ha' somente um intervalo de espacos entre duas palavras com
    ...
    for j in range(0, len(file_str)):
        if (not file_str[j].isalnum()):
            if (strBool):
                ...
                if (j - i >= 6):
                    if (boolPhrase):
                        short = file_str[iPhrase+1:jPhrase]+' '+file_str[i+1:j]
                        if short in freq:
                            temp = freq.get(short)
                            freq.pop(short)
                            freq[short] = temp + 1
    ...
    wordCount = word_count(filename) - 1
...
def is_well_formed(filename):
    """
    Neste algoritmo, uma lista foi usada como uma pilha, todas
    aberturas '{', '[' ou '(', quando apareciam na iteracao, eram
    empilhadas na pilha. Caso aparecesse um fechamento '}', ']' ou
    ')', desempilha-se e compara-se para saber se o respectivo item
    desempilhado corresponde ao seu simbolo fechado na iteracao. 
    Caso negativo, o codigo retorna falso. Alem disso, se ao termino
    do algoritmo a pilha n estiver vazia, quer dizer que algum dos
    simbolos nao foi fechado, logo retorna falso tambem
    ...
    for i in range(0, len(file_str)):
        if (file_str[i] == '[' or file_str[i] == '{' or file_str[i] == '('):
            stack.append(file_str[i])
        elif (file_str[i] == ']'):
...
class PriorityQueue(object):
    # Implementacao de uma fila de prioridade simples
    def __init__(self): 
        self.queue = [] 
...
def uniform_cost_search(problem):
    """ 
    O algoritmo implementa uma fila de prioridade para pegar sempre
    a acao de menor custo total, ou seja, implementando o UCS

    """
    queue = PriorityQueue()
    queue.insert(Node(problem.start(), 0))

    visited = set([])
    while queue:
        node = queue.delete() # remove o no' com menor custo
        ...
        for action in problem.actions(node.state):
            ...
            if next_state not in visited:
                ...
                queue.insert(Node(next_state, cost, node, action))
    return None
