...
class RandomAgent(util.Agent):
    ...
    def get_action(self, perception):
        ...
        total_actions = []
        player_n = self.player_number
        player_nn = -2

        if player_n == 1 or player_n == 8:
            player_nn = 2
        else:
            player_nn = 1
        ...
        for i in range(0, len(grid)):
            for j in range(0, len(grid[0])):
                if grid[i][j] == player_n or grid[i][j] == player_n+7:
                    player_i = i
                    player_j = j
        ...
        if perception[1] > 0:
            ...
            if player_i != 0:
                test = grid[player_i-1][player_j]
                if test not in (5, player_nn, player_nn+7):
                    total_actions.append('UP')

            if player_i != len(grid)-1:
                test = grid[player_i+1][player_j]
                if test not in (5, player_nn, player_nn+7):
                    total_actions.append('DOWN')

            if player_j != 0:
                test = grid[player_i][player_j-1]
                if test not in (5, player_nn, player_nn+7):
                    total_actions.append('LEFT')

            if player_j != len(grid[0])-1:
                test = grid[player_i][player_j+1]
                if test not in (5, player_nn, player_nn+7):
                    total_actions.append('RIGHT')

        if grid[player_i][player_j] == player_n + 7:
            total_actions.append('REFILL')

        total_actions.append('STOP')
...
class CollectAllAgent(util.Agent):
        ...
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                if grid[i][j] in player_values:
                    return (tuple(map(tuple, grid)), (i, j), remaining_gas, 0)
        return None


    def start_agent(self, perception, problem, pp, **kwargs):
        ...
        self.problem = problem(new_grid, self.initial_state, pp, **kwargs)
    ...
    def get_action(self, perception):
        ...
        heuristics.
        """
        self.start_agent(perception, self.problem_reference, 0,
...
class CollectAllAgentProblem(util.Problem):
    ...
    """
    def __init__(self, grid, initial_state, pp, **kwargs):
        self.grid = copy.deepcopy(grid)
        self.init_state = initial_state
        self.grid_height = len(self.grid)
        self.grid_width = len(self.grid[0])
        self.people_position = self.__all_people(initial_state)
    ...
    def __process_state(self, state):
        ...
        :rtype: <class 'tuple'>
        """
        tup, agent_pos, remaining_gas, pp = state
        grid = [list(i) for i in tup]
        i, j = agent_pos
        grid_number = grid[i][j]
        ...
        if player_number == 1:
            ...
            obstacles = [1, 5, 8]
        tup = tuple(map(tuple, grid))
        full_info = (tup, agent_pos, player_number, obstacles, remaining_gas, pp)
        return full_info
    ...
    def __all_people(self, state):
        ...
        for i in range(self.grid_height):
            for j in range(self.grid_width):
                if state[0][i][j] in people_numbers:
                    people_pos[(i, j)] = state[0][i][j]
        return people_pos


    def actions(self, state):
        """ Returns a list of valid actions for a given state """
        tup, ag_pos, pl_number, obstacles, rem_gas, pp = self.__process_state(state)
        grid = [list(i) for i in tup]
        ...
        if rem_gas > 0:
            if i-1 >= 0 and grid[i-1][j] not in obstacles:
                valid.append('UP')
            if j+1 < self.grid_width and grid[i][j+1] not in obstacles:
                valid.append('RIGHT')
            if i+1 < self.grid_height and grid[i+1][j] not in obstacles:
                valid.append('DOWN')
    ...
    def next_state(self, state, action):
        ...
        tup, ag_pos, player_number, _, remaining_gas, pp = self.__process_state(state)
        grid = [list(i) for i in tup]
        ...
        if action not in self.actions(state):
            tup1 = tuple(map(tuple, grid))
            return (tup1, aux['STOP'], remaining_gas - self.cost(state, action), pp)
        new_i, new_j = aux[action]
        if action == 'REFILL':
            if remaining_gas + util.DEFAULT_REFILL > self.tank_capacity:
                grid[new_i][new_j] = player_number + 7
                tup2 = tuple(map(tuple, grid))
                return (tup2, aux['REFILL'], self.tank_capacity, pp)
            else:
                grid[new_i][new_j] = player_number + 7
                tup3 = tuple(map(tuple, grid))
                return (tup3, aux['REFILL'], remaining_gas + util.DEFAULT_REFILL, pp)
        if grid[new_i][new_j] == 4:  # Agent going to a gas station
            grid[new_i][new_j] = player_number + 7
        else:
            people_numbers = [3, 6, 7]
            if grid[new_i][new_j] in people_numbers:
                pp += 1
                if pp > self.ppAlcan:
                    self.ppAlcan = pp
            grid[new_i][new_j] = player_number
        tup4 = tuple(map(tuple, grid))
        return (tup4, aux[action], remaining_gas - self.cost(state, action), pp)
    ...
    def is_goal_state(self, state):
        ...
        """
        if self.pp == 0:
            if len(self.__all_people(state)) > 0:
                return False
        else:
            if state[3] < self.pp:
                return False
